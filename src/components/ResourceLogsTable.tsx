...
import { 
  ...
  Calendar, Mail, User, Phone, Inbox, Copy, Check, FileText 
} from 'lucide-react';

export const ResourceLogsTable = ({ logs, isLoading }: { logs: any[], isLoading: boolean }) => {
  ...
  const handleExportCsv = () => {
    ...
    const headers = ['S.No.', 'Full Name', 'Email', 'Phone', 'Resource Downloaded', 'Date'];
    ...
    link.href = url;
    link.download = `resource-logs-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
      ...
      <div className="p-4 sm:p-5 border-b border-slate-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="Search logs by name, email, phone, or resource..."
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50/80 text-[11px] font-bold uppercase tracking-wider text-slate-600">
              ...
              <th className="py-3 px-4 cursor-pointer hover:bg-slate-100 transition select-none" onClick={() => handleSort('mobile')}>
                <div className="flex items-center gap-1.5"><Phone className="w-3.5 h-3.5 text-slate-400" /><span>Phone</span></div>
              </th>
              <th className="py-3 px-4 cursor-pointer hover:bg-slate-100 transition select-none" onClick={() => handleSort('resourceName')}>
                <div className="flex items-center gap-1.5"><FileText className="w-3.5 h-3.5 text-slate-400" /><span>Resource Name</span></div>
              </th>
          ...
          <tbody className="divide-y divide-slate-100 text-xs text-slate-800">
            {isLoading ? (
              <tr><td colSpan={6} className="py-12 text-center text-slate-500">Loading logs...</td></tr>
              ...
              paginatedLogs.map((log, index) => {
                ...
                return (
                  <tr key={index} className="hover:bg-slate-50/75 transition-colors group">
                    ...
                    <td className="py-3 px-4 text-slate-700">
                      <div className="flex items-center gap-1.5 group/copy">
                        <span>{log.email}</span>
                        <button onClick={() => handleCopy(log.email, 'email')} className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-slate-700 rounded transition"><Copy className="w-3 h-3" /></button>
                      </div>
                    </td>
                    <td className="py-3 px-4 text-slate-700">
                      <div className="flex items-center gap-1.5 group/phone">
                        <span className="font-mono">{log.mobile || '-'}</span>
                        {log.mobile && <button onClick={() => handleCopy(log.mobile, 'phone')} className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-slate-700 rounded transition"><Copy className="w-3 h-3" /></button>}
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <span className="px-2.5 py-1 rounded-md bg-amber-50 text-amber-700 border border-amber-200 font-medium text-[11px]">{log.resourceName}</span>
                    </td>
