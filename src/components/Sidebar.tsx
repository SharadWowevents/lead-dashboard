...
import { Layers, Globe, Filter, Code2, PlusCircle, Search, ChevronRight } from 'lucide-react';
  ...
  const sidebarContent = (
    <div className="h-full flex flex-col justify-between p-4 bg-slate-900 text-slate-200">
      <div className="space-y-6">
        ...
        {/* Quick Test Ingest Action */}
        {/* <div className="px-1">
          <button
            type="button"
            id="open-ingest-tester-btn"
            onClick={onOpenIngestTester}
            className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-750 border border-slate-700/80 text-xs font-semibold text-slate-200 hover:text-white transition group cursor-pointer shadow-xs"
          >
            <span className="flex items-center gap-2">
              <Code2 className="w-4 h-4 text-emerald-400" />
              <span>Ingest API & Tester</span>
            </span>
      ...
      <div className="p-3 rounded-xl bg-slate-800/80 border border-slate-700/60 text-xs">
        <div className="flex items-center justify-between text-slate-300 font-medium mb-1">
          <span className="text-[11px] text-slate-400 uppercase tracking-wider">Backend Status</span>
